...
class NotificationService {
  constructor() {
    ...
    this.seenDataPath = path.join(
      app.getPath("userData"),
      "seen-notifications.json",
    );
  ...
  loadSeenData() {
    try {
      if (fs.existsSync(this.seenDataPath)) {
        ...
        if (
          Array.isArray(data.seenNotificationIds) &&
  ...
  cleanupOldNotifications() {
    if (
      !this.seenData.seenNotificationIds ||
      !Array.isArray(this.seenNotificationIds)
    ) {
      return;
    }
    ...
    this.seenData.seenNotificationIds =
      this.seenData.seenNotificationIds.filter((item) => {
    ...
    if (originalLength !== this.seenData.seenNotificationIds.length) {
      console.log(
        `Cleaned up ${originalLength - this.seenData.seenNotificationIds.length} old notifications`,
      );
      this.saveSeenData();
    }
  }
  ...
  getAppNotifications() {
    return new Promise((resolve, reject) => {
      ...
      if (process.env["deepnest_debug"] === "1") {
        try {
          ...
          if (fs.existsSync(examplePath)) {
            ...
            console.warn(
              "Debug mode: Example notifications file not found at",
              examplePath,
            );
          }
        } catch (err) {
      ...
      https
        .get("https://www.deepnest.net/app_notifications.json", (res) => {
          let data = "";
          res.on("data", (chunk) => (data += chunk));
          res.on("end", () => {
            if (res.statusCode === 200) {
              ...
              reject(
                new Error(
                  `Notifications API responded with status code ${res.statusCode}`,
                ),
              );
            }
          });
  ...
  /**
   * Check for new app notifications and releases
   * Main entry point for notification checking - combines release checks and notification fetch
   * @returns Promise resolving to notification object or null if no new notifications
   * @sideEffect Updates seen notifications data in local storage if new notifications found
   */
  async checkForNotifications() {
    try {
      ...
      const notificationsResponse = await this.getAppNotifications().catch(
        () => ({ notifications: [] }),
      );
      ...
      const seenUuids = this.seenData.seenNotificationIds.map(
        (item) => item.uuid,
      );
      ...
      if (unseenNotifications.length > 0) {
        // Group by type
        const importantAndUpdate = unseenNotifications.filter(
          (n) => n.type === "important" || n.type === "update",
        );
        const otherNotifications = unseenNotifications.filter(
          (n) => n.type !== "important" && n.type !== "update",
        );
            ...
            importantAndUpdate.forEach((notification) => {
              content += `
                                <div class="merged-notification">
                                    ...
                                    <p class="notification-date">Date: ${new Intl.DateTimeFormat(
                                      "en-US",
                                      {
                                        dateStyle: "full",
                                        timeStyle: "long",
  ...
  /**
   * Format release information for UI display
   * @param release - GitHub release object with name, body, assets, etc.
   * @returns HTML string with formatted release notes and download links
   * @sideEffect None (returns new string)
   */
   ...
   * @returns Formatted string like "2.45 MB" or "1.23 KB"
   * @sideEffect None
   */
