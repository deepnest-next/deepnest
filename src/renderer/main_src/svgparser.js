  ...
  for (i = 0; i < openpaths.length; i++) {
    ...
    while (c) {
      ...
      if (this.isClosed(merged, tolerance)) {
        let lastCommand = merged.pathSegList.getItem(
  ...
  for (var i = 0; i < paths.length; i++) {
    ...
    if (path.tagName == 'polyline' || path.tagName == 'polygon') {
      ...
      for (var j = 0; j < path.points.length - 1; j++) {
        ...
        let p2 = path.points[j + 1]
        addLine(p1.x, p1.y, p2.x, p2.y)
      }

      if (path.tagName == 'polygon') {
        let p1 = path.points[path.points.length - 1]
        let p2 = path.points[0]
        addLine(p1.x, p1.y, p2.x, p2.y)
      }
  ...
  var topath = function (svg, p) {
    if (p.tagName == 'line') {
      ...
      pa.pathSegList.appendItem(
        pa.createSVGPathSegMovetoAbs(Number(p.getAttribute('x1')), Number(p.getAttribute('y1')))
      )
      pa.pathSegList.appendItem(
        pa.createSVGPathSegLinetoAbs(Number(p.getAttribute('x2')), Number(p.getAttribute('y2')))
      )
...
SvgParser.prototype.isClosed = function (p, tolerance) {
  ...
  if (p.tagName == 'polyline') {
    ...
    if (
      GeometryUtil.almostEqual(first.x, last.x, tolerance || this.conf.toleranceSvg) &&
      GeometryUtil.almostEqual(first.y, last.y, tolerance || this.conf.toleranceSvg)
    ) {
      return true
    } else {
      return false
  ...
  if (element.tagName == 'g' || element.tagName == 'svg' || element.tagName == 'defs') {
    ...
    switch (element.tagName) {
      case 'ellipse':
        ...
        {
          let transformProperty = element.getAttribute('transform')
        ...
        element = path
        break // is this break necessary? I have added it, because the code below is not working for ellipses
      case 'path':
        ...
        for (let i = 0; i < seglist.numberOfItems; i++) {
          ...
          if ('x' in s && 'y' in s) {
            let transformed = transform.calc(s.x, s.y)
            prevx = s.x
            prevy = s.y
          ...
          if ('x1' in s && 'y1' in s) {
            let transformed = transform.calc(s.x1, s.y1)
            s.x1 = transformed[0]
            s.y1 = transformed[1]
          }
          if ('x2' in s && 'y2' in s) {
            let transformed = transform.calc(s.x2, s.y2)
            s.x2 = transformed[0]
            s.y2 = transformed[1]
          }
        }
      ...
      case 'circle': {
        ...
        let transformed = transform.calc(element.getAttribute('cx'), element.getAttribute('cy'))
...
SvgParser.prototype.polygonifyPath = function (path) {
  ...
  for (var i = 0; i < seglist.numberOfItems; i++) {
    ...
    switch (command) {
      ...
      case 'c':
      case 'C': {
        let pointlist = GeometryUtil.CubicBezier.linearize(
          { x: prevx, y: prevy },
          { x: x, y: y },
          { x: x1, y: y1 },
          { x: x2, y: y2 },
      ...
      case 'a':
      case 'A': {
        let pointlist = GeometryUtil.Arc.linearize(
          { x: prevx, y: prevy },
          { x: x, y: y },
          s.r1,
          s.r2,
