...
function DeepNest(eventEmitter) {
  ...
  this.importsvg = function (filename, dirpath, svgstring, scalingFactor, dxfFlag) {
    ...
    svg = window.SvgParser.load(dirpath, svgstring, config.scale, scalingFactor)
    svg = window.SvgParser.clean(dxfFlag)
  ...
  this.getParts = function (paths, filename) {
    ...
    for (i = 0; i < polygons.length; i++) {
      ...
      if (window.config.getSync('useQuantityFromFileName') && part.filename !== null) {
  ...
  this.cleanPolygon = function (polygon) {
    ...
    if (
      start == end ||
      (GeometryUtil.almostEqual(start.x, end.x) && GeometryUtil.almostEqual(start.y, end.y))
    ) {
      cleaned.pop()
    }
  ...
  // TODO: this function is not in use, but could be useful for debugging
...
window.DeepNest = new DeepNest(window.electron.ipcRenderer)
