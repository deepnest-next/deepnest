import { ConsoleMessage, _electron as electron, expect, test } from '@playwright/test'
...
declare global {
  interface Window {
    config: {
      getSync: (arg?: string) => DeepNestConfig
    }
    DeepNest: {
      nests: NestingResult
...
test('Nest', async ({}, testInfo) => {
  ...
  !existsSync(testInfo.outputDir) && mkdir(testInfo.outputDir, { recursive: true })
  ...
  pipeConsole &&
    test.step(
      ...
      () => {
        ...
        appWindow.on('console', logMessage)
  ...
  await test.step('Config', async () => {
    await appWindow.locator('#config_tab').click()
    const configTab = appWindow.getByText('Nesting configuration Display')
    await configTab.getByRole('link', { name: 'set all to default' }).click()
    await test.step('units mm', () => configTab.getByRole('radio').nth(1).check())
    ...
    await test.step('placement type gravity', () =>
      configTab.locator('select[name="placementType"]').selectOption('gravity'))
    const config = await appWindow.evaluate(() => {
      return window.config.getSync()
    })
    const deepNestConfig = await appWindow.evaluate(() => {
    ...
    await appWindow.locator('#home_tab').click()
  })
  ...
  await test.step('Upload files', async () => {
    ...
    await appWindow.click('id=import')
    await expect(appWindow.locator('#importsnav li')).toHaveCount(2)
  })

  await test.step('Add sheet', async () => {
    const sheet = { width: 300, height: 200 }
    await appWindow.click('id=addsheet')
    await appWindow.fill('id=sheetwidth', sheet.width.toString())
    await appWindow.fill('id=sheetheight', sheet.height.toString())
    await appWindow.click('id=confirmsheet')
  })

  // await expect(window).toHaveScreenshot("loaded.png", {
  //   clip: { x: 100, y: 100, width: 2000, height: 1000 },
  // });
  await appWindow.click('id=startnest')

  const stopNesting = () =>
    test.step('Stop nesting', async () => {
      const button = appWindow.locator('#stopnest')
      await button.click()
  ...
  const downloadSvg = () =>
    test.step('Download SVG', async () => {
      ...
      await appWindow.click('id=export')
      await expect(appWindow.locator('id=exportsvg')).toBeVisible()
      await appWindow.click('id=exportsvg')
  ...
  await expect(appWindow.locator('id=progressbar')).toBeVisible()
  await waitForIteration(1)
  await expect(appWindow.locator('id=nestinfo').locator('h1').nth(0)).toHaveText('1')
  await expect(() =>
    expect(appWindow.locator('id=nestinfo').locator('h1').nth(1)).toHaveText('54/54')
  ).toPass()

  await test.step('Attachments', async () => {
    const svg = await downloadSvg()
    const data = (): Promise<NestingResult> => appWindow.evaluate(() => window.DeepNest.nests)
