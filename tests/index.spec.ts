...
test('Nest', async ({}, testInfo) => {
  ...
  pipeConsole &&
    test.step(
      ...
      () => {
        const logMessage = async (message: ConsoleMessage): Promise<void> => {
          await test.step(
            'Log message',
            async () => {
  ...
  const waitForIteration = (n: number): Promise<void> =>
  ...
  await expect(() =>
    expect(window.locator('id=nestinfo').locator('h1').nth(1)).toHaveText('54/54')
  ).toPass()

  await test.step('Attachments', async () => {
    const svg = await downloadSvg()
    const data = (): Promise<NestingResult> => window.evaluate(() => window.DeepNest.nests)
